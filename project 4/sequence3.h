// PROVIDED BY: ________________ (Yu Bin Liu yliu009@citymail.cuny.edu)
// FILE: sequence3.h
...
namespace main_savitch_5
{
    ...
    public:
        ...
        value_type current( ) const;
    private:
        node *head_ptr; // the head_ptr points to the first element of the link list
        node *tail_ptr; // the tail_ptr points to the last element of the link list
        node  *precursor; // the node that points to the current node (previous node in a sense)
        node *cursor; // the current node
